...
@Autonomous(name="Challenge Colors Runner V2.3", group="Challenge Gay")
public class ThreeCornersV2 extends OpMode
{
    ...
    public void start()
    {
        switch (Objects.requireNonNull(this.getThreeColor(colorSensor.getNormalizedColors())))
        {
            case "white":
                distance = 100;
                leftDrive.setTargetPosition((int) (((leftDrive.getCurrentPosition() - (TURN_RATE * LEFT_ENCODER_RATE)) + distance * LEFT_ENCODER_RATE)));
                rightDrive.setTargetPosition((int) (((rightDrive.getCurrentPosition() + (TURN_RATE * RIGHT_ENCODER_RATE)) + distance * RIGHT_ENCODER_RATE)));
                telemetry.addData("Moving", "RED");
            case "blue":
                distance = 100;
                leftDrive.setTargetPosition((int) (((leftDrive.getCurrentPosition() + (TURN_RATE * LEFT_ENCODER_RATE)) + distance * LEFT_ENCODER_RATE)));
                rightDrive.setTargetPosition((int) (((leftDrive.getCurrentPosition() - (TURN_RATE * RIGHT_ENCODER_RATE)) + distance * RIGHT_ENCODER_RATE)));
                telemetry.addData("Moving", "BLUE");
            case "red":
                distance = 140;
                leftDrive.setTargetPosition((int) (leftDrive.getCurrentPosition() + (distance * LEFT_ENCODER_RATE)));
                rightDrive.setTargetPosition((int) (rightDrive.getCurrentPosition() + (distance * RIGHT_ENCODER_RATE)));
    ...
    public void loop()
    {
        ...
        if (rightDrive.getCurrentPosition() == rightDrive.getCurrentPosition())
        {
